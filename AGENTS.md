# Code Lesson Checkpoints (`code-lesson-checkpoints`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://code-lesson-checkpoints.sociobot.in`. Artifact class: `web-with-backend`.
...
- Keep the stack boring and fast: Vite + vanilla TS or Svelte/Preact for static; Rust (axum) or Node (Hono) for backends; SQLite for data. No paid third-party services.
