...
- Isolation: the backend provisions a random, separately namespaced `demo_workspaces` record with a 24-hour expiry. It never writes demo rows to the real `lessons`, `checkpoints`, or `submissions` tables. This keeps the demo usable across replicas. The browser caches the sample only under `demo:clc:workspace`; real `clc:*` lesson and `sb_license:*` keys are not read or changed.
